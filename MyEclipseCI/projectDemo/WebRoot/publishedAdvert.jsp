...
<%@page import="com.etc.dao.AdminDao"%>
...
<body>
 <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  <legend align="left" style="color:green;">广告信息/已审核</legend>
</fieldset>
...
<table class="layui-table">
  ...
  <tr>
  	...
  	<c:set var="advertName" value="${advert.advertName}" scope="request"></c:set>
  	 <c:set var="adminID" value="${advert.adminID.adminID}" scope="request"></c:set>
      ...
      <td id="imgdiv" onclick="imgbig('${advert.advertName}');"><img src='<%="image\\\\ad\\\\"+(String)request.getAttribute("advertName") %>' id="imgsrc"/></td>
      	 <td><font color="green">${advert.checkStatus}</font></td>
	 <c:if test="${adminName eq '龙金河' }">
      <td><%=adminDao.getAdminName((int)request.getAttribute("adminID")) %></td>
      </c:if>
      <td><button type="button" class="layui-btn layui-btn-danger" onclick="deleteAdvert(${advert.advertID})">删除广告</button></td>
    </tr>
     </c:forEach>
  <tr>
   		<td colspan="7">
...
<c:if test="${pagedata.totalPage == 0}">
      	 <center><font size="30px" color="black">暂时没有已审核的广告！</font></center>
</c:if>
...
//首页
  ...
  location.href="AdvertServlet?option=queryByPageAdmin&pageNo=1&pageSize="+pageSize+"&url=publishedAdvert.jsp&checkStatus=已审核";
  }
  ...
  if(pageNo<=1){
  	...
  	location.href="AdvertServlet?option=queryByPageAdmin&pageNo="+(pageNo-1)+"&pageSize="+pageSize+"&url=publishedAdvert.jsp&checkStatus=已审核";
  }
  ...
  if(pageNo>=totalPage){
  	...
  	location.href="AdvertServlet?option=queryByPageAdmin&pageNo="+totalPage+"&pageSize="+pageSize+"&url=publishedAdvert.jsp&checkStatus=已审核";
  }
  function search(){
  ...
  location.href="AdvertServlet?option=queryLikeNameAdmin&checkStatus=已审核&url=publishedAdvert.jsp&advertID="+advertID;
  }
  
  function refresh(){
  var positionName = document.getElementById("btn_text").value;
  location.href="AdvertServlet?option=queryByPageAdmin&&&url=publishedAdvert.jsp&checkStatus=已审核";
  }
   function show () {
    ...
    function deleteAdvert(advertID){
        location.href="AdvertServlet?option=deleteAdvertAdmin&url=publishedAdvert.jsp&checkStatus=已审核&advertID="+advertID;
    }
    </script>
