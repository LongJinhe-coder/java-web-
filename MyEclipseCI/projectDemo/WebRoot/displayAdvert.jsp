...
<body>
 <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  <legend align="left" style="color:green;">发布广告/已发布</legend>
</fieldset>
...
<table class="layui-table">
  ...
  <c:set var="advertName" value="${advert.advertName}" scope="request"></c:set>
      ...
      <td><img src='<%="image\\\\ad\\\\"+(String)request.getAttribute("advertName") %>'/></td>
        <c:if test="${advert.checkStatus eq '已审核'}">
      	 <td><font color="green">${advert.checkStatus}</font></td>
      </c:if>
     <c:if test="${advert.checkStatus eq '未审核'}">
      	 <td><font color="red">${advert.checkStatus}</font></td>
      </c:if>

      <td><button type="button"  class="layui-btn layui-btn-danger" onclick="deleteInfo(${advert.advertID})"><i class="layui-icon layui-icon-reduce-circle" ></i>删除广告</button></td>
    </tr>
...
//首页
  ...
  location.href="AdvertServlet?option=queryByPage&pageNo=1&pageSize="+pageSize+"&url=displayAdvert.jsp";
  }
  ...
  if(pageNo<=1){
  	...
  	location.href="AdvertServlet?option=queryByPage&pageNo="+(pageNo-1)+"&pageSize="+pageSize+"&url=displayAdvert.jsp";
  }
  ...
  if(pageNo>=totalPage){
  	...
  	location.href="AdvertServlet?option=queryByPage&pageNo="+totalPage+"&pageSize="+pageSize+"&url=displayAdvert.jsp";
  }
  function search(){
  ...
  location.href="AdvertServlet?option=queryLikeName&url=displayAdvert.jsp&advertID="+advertID;
  }
  
  function refresh(){
  var positionName = document.getElementById("btn_text").value;
  location.href="AdvertServlet?option=queryByPage&url=displayAdvert.jsp";
  }
   function show () {
    ...
    function deleteInfo(advertID){
        location.href="AdvertServlet?option=deleteInfo&url=displayAdvert.jsp&advertID="+advertID;
    }
    </script>
</html>
