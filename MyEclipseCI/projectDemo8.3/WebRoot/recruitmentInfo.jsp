  ...
  <div class="layui-form-item">
    <div class="layui-inline">
      <div class="layui-input-inline" style="margin-left:400px">
        <input onkeydown="show();" type="text" name="positionName" autocomplete="off"  id="btn_text" class="layui-input" placeholder="请输入要搜索的岗位名称">
      </div>
...
<table class="layui-table">
  ...
  <tr>
      ...
      <td>${recruitment.dueDate}</td>
        <c:if test="${recruitment.checkStatus eq '已审核'}">
      	 <td><font color="green">${recruitment.checkStatus}</font></td>
      </c:if>
     <c:if test="${recruitment.checkStatus eq '未审核'}">
...
//传pageNo,pageSize是为了删除后回到当前界面
 ...
 //首页
  ...
  location.href="RecruitmentInfo?option=queryByPage&pageNo=1&pageSize="+pageSize+"&url=recruitmentInfo.jsp";
  }
  ...
  if(pageNo<=1){
  	...
  	location.href="RecruitmentInfo?option=queryByPage&pageNo="+(pageNo-1)+"&pageSize="+pageSize+"&url=recruitmentInfo.jsp";
  }
  ...
  if(pageNo>=totalPage){
  	...
  	location.href="RecruitmentInfo?option=queryByPage&pageNo="+totalPage+"&pageSize="+pageSize+"&url=recruitmentInfo.jsp";
  }
  
  function deleteInfo(recruitmentID,pageNo,pageSize){
	location.href="RecruitmentInfo?option=deleteInfo&recruitmentID="+recruitmentID+"&pageNo="+pageNo+"&pageSize="+pageSize+"&url=recruitmentInfo.jsp";
}

 function lookInfo(recruitmentID,pageNo,pageSize){
   location.href="RecruitmentInfo?option=lookInfo&recruitmentID="+recruitmentID+"&pageNo="+pageNo+"&pageSize="+pageSize+"&url=displayRecruitmentInfo.jsp";
 }
  ...
  location.href="RecruitmentInfo?option=queryLikeName&positionName="+positionName+"&url=recruitmentInfo.jsp";
  }
  
  function refresh(){
  var positionName = document.getElementById("btn_text").value;
  location.href="RecruitmentInfo?option=queryByPage&positionName="+positionName+"&url=recruitmentInfo.jsp";
  }
   function show () {
