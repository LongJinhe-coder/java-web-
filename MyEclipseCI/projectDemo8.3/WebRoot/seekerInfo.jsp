  ...
  <div class="layui-form-item">
    <div class="layui-inline">
      <div class="layui-input-inline" style="margin-left:400px">
        <input onkeydown="show();" type="text" name="positionName" autocomplete="off"  id="btn_text" class="layui-input" placeholder="请输入要搜索的简历编号">
      </div>
...
<table class="layui-table">
  ...
  <tr>
	 ...
	 recruitment2=(Recruitment)pagedata.getData().get(count++);
     //得到招聘信息
	   System.out.println("positionID-------**---->"+recruitment2.getRecruitmentID().getRecruitmentID());
	  RecruitmentInformation ri=dao.queryRecruitmentInfoById(recruitment2.getRecruitmentID().getRecruitmentID());
	  //得到岗位名称
	  String positionName=dao.getPositionName(ri.getPositionID().getPositionID());
     ...
     <c:if test="${recruitment.handleStatus eq '未应聘'}">
      ...
      <button type="button" class="layui-btn layui-btn-danger" onclick="deleteInfo(${recruitment.userInfoID.userInfoID},<%=recruitment2.getRecruitmentID().getRecruitmentID() %>,${pagedata.pageNo},${pagedata.pageSize})">删除简历</button>
        
        <c:if test="${recruitment.handleStatus eq '未应聘'}">	
       <button type="button" class="layui-btn layui-btn-normal" onclick="doAccept(${recruitment.userInfoID.userInfoID},<%=recruitment2.getRecruitmentID().getRecruitmentID() %>,${pagedata.pageNo},${pagedata.pageSize})">同意应聘</button>
       </c:if>
       
       <c:if test="${recruitment.handleStatus eq '已应聘'}">	
       <button type="button" class="layui-btn layui-btn-warm" onclick="doReject(${recruitment.userInfoID.userInfoID},<%=recruitment2.getRecruitmentID().getRecruitmentID() %>,${pagedata.pageNo},${pagedata.pageSize})">取消应聘</button>
       </c:if>
      </td>
    </tr>
...
//传pageNo,pageSize是为了删除后回到当前界面
 ...
 //首页
  ...
  function deleteInfo(userInfoID,recruitmentID,pageNo,pageSize){

	location.href="SeekerInfo?option=deleteInfo&userInfoID="+userInfoID+"&pageNo="+pageNo+"&pageSize="+pageSize+"&url=seekerInfo.jsp"+"&recruitmentID="+recruitmentID;
}
 ...
 function lookInfo(userInfoID,pageNo,pageSize){
  ...
  location.href="SeekerInfo?option=queryByPage&url=seekerInfo.jsp";
  }
   function show () {
    ...
    function doAccept(userInfoID,recruitmentID,pageNo,pageSize){
    //alert("accept");

       location.href="SeekerInfo?option=accept&userInfoID="+userInfoID+"&pageNo="+pageNo+"&pageSize="+pageSize+"&url=seekerInfo.jsp"+"&recruitmentID="+recruitmentID;
    }
    
    function doReject(userInfoID,recruitmentID,pageNo,pageSize){

    location.href="SeekerInfo?option=reject&userInfoID="+userInfoID+"&pageNo="+pageNo+"&pageSize="+pageSize+"&url=seekerInfo.jsp"+"&recruitmentID="+recruitmentID;
    }
