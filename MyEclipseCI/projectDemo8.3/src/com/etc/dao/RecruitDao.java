...
public class RecruitDao {
   ...
   //删除应聘信息/删除简历
 	public boolean deleteResumeInfo(int userInfoID,int recruitmentID) {
 		int result = 0;
 		String sql = "delete from 应聘表 where userInfoID = ? and recruitmentID= ?";
	...
	//模糊查询招聘信息
		...
		public PageData queryLikeID(int userInfoID,int firmID) {
			 ...
			 String sql = "SELECT a.userInfoID,a.recruitmentID,a.handleStatus from 应聘表 a,招聘信息表  b where b.firmID=? and a.recruitmentID=b.recruitmentID and a.userInfoID like '"+userInfoID+"'";
			//调用dbutil的getpage方法
		...
		//应聘信息的分页查询
		  public PageData queryByPage2(int pageNo,int pageSize,int firmID) {
				...
				String sql ="SELECT a.userInfoID,a.recruitmentID,a.handleStatus,c.positionName,b.positionID from 应聘表 a,招聘信息表  b,职位表 c where b.firmID=? and a.recruitmentID=b.recruitmentID and b.positionID=c.positionID";
	...
	//获取企业地址
		...
		//同意应聘
				public boolean accept(int userInfoID,int recruitmentID) {
					int result = 0;
					String status = "已应聘";
					String sql = "update 应聘表 a,招聘信息表 b set a.handleStatus = ? where a.userInfoID = ? and a.recruitmentID=?";
				...
				//拒绝应聘
						public boolean reject(int userInfoID,int recruitmentID) {
							int result = 0;
							String status = "未应聘";
							String sql = "update 应聘表 a,招聘信息表 b set handleStatus = ? where a.userInfoID = ? and a.recruitmentID=?";
							result = dbutil.update(sql,status,userInfoID,recruitmentID);
							return result>0;
		...
		public List<Position> queryAllPosition() {
			String sql = "select * from 职位表 ";
			List<Position> list = (List<Position>)dbutil.Select(sql, Position.class);
			System.out.println("职位表长度------------->"+list.size());
			return list;
