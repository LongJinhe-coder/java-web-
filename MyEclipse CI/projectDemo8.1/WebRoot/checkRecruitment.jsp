...
<%
 ...
 <fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
  <legend align="left" style="color:green;">招聘信息/未审核</legend>
</fieldset>
...
<table class="layui-table">
  ...
  <tr>
      ...
      <c:set var="firmID" value="${recruitment.firmID.firmID}" scope="request"></c:set>
      <td><%=rdao.getFirmName((int)request.getAttribute("firmID"))%></td>
      <!-- 得到岗位名称 -->
      <td><%=rdao.getPositionName((int)request.getAttribute("positionID"))%></td>
      <td>${recruitment.count}</td>
      <td>${recruitment.salary}</td>
      <td><font color="red">${recruitment.checkStatus}</font></td>
      <td><button type="button" class="layui-btn layui-btn-normal" onclick="checkStatus(${recruitment.recruitmentID})">进入审核</button></td>
    </tr>
...
<c:if test="${pagedata.totalPage == 0}">
      	 <center><font size="30px" color="black">暂时没有需要审核的招聘信息！</font></center>
</c:if>
</body>
<script type="text/javascript">
function checkInfo(firmID){

	location.href="RecruitmentInfo?option=queryByID&firmID="+firmID+"&url=displayFirmResume.jsp&checkStatus=未审核";
}

function checkStatus(recruitmentID){
 ...
 //首页
  ...
  location.href="RecruitmentInfo?option=queryByPageAdmin&pageNo=1&pageSize="+pageSize+"&url=checkRecruitment.jsp&checkStatus=未审核";
  }
  ...
  if(pageNo<=1){
  	...
  	location.href="RecruitmentInfo?option=queryByPageAdmin&pageNo="+(pageNo-1)+"&pageSize="+pageSize+"&url=checkRecruitment.jsp&checkStatus=未审核";
  }
  ...
  if(pageNo>=totalPage){
  	...
  	location.href="RecruitmentInfo?option=queryByPageAdmin&pageNo="+totalPage+"&pageSize="+pageSize+"&url=checkRecruitment.jsp&checkStatus=未审核";
  }
 ...
 var positionName = document.getElementById("btn_text").value;
  ...
  location.href="RecruitmentInfo?option=queryLikeNameAdmin&positionName="+positionName+"&url=checkRecruitment.jsp&checkStatus=未审核";
  }
  
  function refresh(){
  location.href="RecruitmentInfo?option=queryByPageAdmin&url=checkRecruitment.jsp&checkStatus=未审核";
  }
   function show () {
